...
def stack_combine(source1, source2):
    ...
    target = Stack() #Init a new target stack
    index = 1 #Define index we are starting at
    
    #Check to make sure the stacks are not empty
    while(not source1.is_empty()) or (not source2.is_empty()):
        #
        if(index % 2 == 0) and (not source2.is_empty()) or source1.is_empty():
            
            value = source2.pop() #Remove the value from source1 stack
            target.push(value) #Push the value onto the top of source1 stack
            
        else:
            value = source1.pop() #Remove the value from source2 stack
            target.push(value) #Push the value onto the top of source2 stack
